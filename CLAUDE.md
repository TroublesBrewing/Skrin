...
- `internal/ui`: the root model (`model.go`), rendering (`view.go`), file actions and marks (`ops.go`), editing, the save-conflict flow, `E` and `u` (`edit.go`), following links, history, backlinks, outline, `[[` completion and moving with link updates (`links.go`), `/` search and the `Ctrl-p` switcher (`find.go`), `R` search & replace (`replace.go`), the name prompt, the y/n question and the filterable chooser used for moves, backlinks and the outline (`modal.go`), the folder tree, and the keymap registry (`keys.go`). Every key goes through the registry, because the `?` manual will be generated from it.
