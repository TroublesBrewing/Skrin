...
func (m *Model) guideText(w int) []manualLine {
	...
	para("Skrin follows the system theme where there is one — Omarchy's, live, as you switch it. Where there isn't, it uses a palette of its own: gruvbox, or flexoki-light, which is paper and ink rather than white glare. Settings chooses which, and a system theme always wins over it.")
	para("Settings ends in a BETA block: experiments, which Skrin doesn't promise to keep. It takes two switches — \"Beta features\" opens the block, and each experiment has its own row inside it — and a release can leave every one of them out at once, however your config reads.")
	...
	para("A link to a note that doesn't exist yet is dimmed; following it creates the note where Obsidian would put it, with no question in the way. The flash names what was made, and U takes it away again.")
	...
	key("this.file.name", "the note the spread sits in, not the row")
	key(`contains(f, "x")`, "a list holding x, or text containing it")
	key("FLATTEN f AS name", "one row per value in a list field")
	...
	para("Any field with this. in front is read from the note the spread is written in rather than the note in the row: WHERE file.name != this.file.name leaves that note out of its own answer, and WHERE author = this.author finds the notes that share its author.")
	para("FLATTEN fans a list field out into one row per value, under the name you give it: FROM this.file FLATTEN file.outlinks AS link lists the links in the note the spread sits in, one per row. A note whose list is empty drops out of the answer.")
