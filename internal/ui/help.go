...
func (m *Model) guideText(w int) []manualLine {
	...
	para("Paper grain, also in that block, lays a faint texture under the note, a shade either side of the theme's own background, so reading feels a little more like a page. It paints the note pane's background, so a transparent terminal turns solid there, and it costs more to draw than plain text: you won't notice locally, you might over ssh. paper.strength in config.toml (1-10, 3 by default) says how far the shades sit from the background.")
