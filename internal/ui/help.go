...
func (m *Model) guideText(w int) []manualLine {
	...
	para("Paper grain, also in that block, lays a texture under the note so reading feels a little more like a page. Settings offers it off, faint, medium or strong: how far the shades sit from the background is what decides whether you see it at all, and three steps of 255 — the first try — is one per cent, which no screen shows. The grain goes darker on a light theme and lighter on a dark one, since that is where the room is. It paints the note pane's background, so a transparent terminal turns solid there, and it costs more to draw than plain text: you won't notice locally, you might over ssh.")
